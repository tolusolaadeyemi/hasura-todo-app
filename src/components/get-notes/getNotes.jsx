 ...
 const deleteNoteId = (note_id,name) => {
  ...
  const markItemAsDone= (id) => {
    ...
    <div className="flex mb-4 items-center">
        ...
        <button id="myFinished" onClick={() => markItemAsDone(item.note_id)}className="shrink w-20 h-10 p-2 mr-9 ml-9 border-2 rounded hover:text-white border-green hover:bg-green">done </button>
        <button onClick={() => deleteNoteId(item.note_id)} className="shrink w-20 h-10 p-2 mb-3  border-2 rounded text-red border-red hover:text-white hover:bg-red">delete</button>
        </li>
        ))}
        </ul>
    </div>
